...
class Facebook:
    ...
    def login_get_cookies(self, user: str, password: str, fa_key: str):
        ...
        if 'submit[Submit Code]' in rq.text:
            ...
            self.login_with_2fa(cookies, fb_dtsg, jazoest, checkpoint_data, nh, code)
    ...
    def submit_login(self, cookies: str, fb_dtsg: str, jazoest: str, checkpoint_data: str, nh: str):
        ...
        rq = self.session.post(url, data=payload, headers=headers, allow_redirects=False)
        
        # get cookies
        cookies = rq.cookies.get_dict()

        # convert dict to string
        self.account_cookies = '; '.join([f'{key}={value}' for key, value in cookies.items()])
